...
const removerMedicao = (async (relatorio, medicao, index) => {
    ...
    if(medicao.id != null) {
        await api.delete('/relatorios/' + relatorio.id + '/medicao/' + medicao.id)
        .then(() => {
...
const salvarAlteracoes = (async (relatorio) => {
    ...
    for(let i = 0; i < relatorio.medicoes.length; i++) {
        delete relatorio.medicoes[i].id;
        delete relatorio.medicoes[i].medicao.id;
    }

    await api.patch('/relatorios/update/' + relatorio.id, relatorio)
    .then(async () => {
        getRelatorios();
        showStatusText.value = true;
        statusText.value = "Alterações salvas com sucesso!";
...
<template>
    <div class="modal modal-lg fade" :id="'editarPacienteModal' + paciente.id" tabindex="-1" aria-labelledby="editarPacienteModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5 editarPacienteeModalLabel">{{ paciente.nomeCompleto }} <div class="btn btn-danger" @click="deletarPaciente()"><i class="bi bi-trash-fill"></i> {{ deletarText }}</div></h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form @submit.prevent="submitForm">
                    <div class="modal-body py-4">
                        <label for="nomePaciente">Nome do Paciente</label>
                        <input class="form-control" :value="paciente.nomeCompleto" id="nomePaciente" type="text" disabled>

                        <div class="row mt-2">
                            <div class="col">
                                <label for="emailPaciente">Email</label>
                                <input class="form-control" :value="paciente.email" id="emailPaciente" type="email" disabled>
                            </div>

                            <div class="col">
                                <label for="telefone">Celular/Telefone</label>
                                <input class="form-control" :value="paciente.telefone" id="telefone" type="text" disabled>
                            </div>
                        </div>
                        <div class="row mt-2">
                            <div class="col">
                                <label for="dataNascimento">Data de Nascimento</label>
                                <input class="form-control" :value="paciente.dataNascimento" id="dataNascimento" type="date"
                                    disabled>
                            </div>
                            <div class="col">
                        ...
                        <div v-for="(relatorio, relatorioIndex) in relatorios.value"
                            :key="'registroDiarioAccordion' + relatorio.id"
                            class="accordion-item">
                            <h2 class="accordion-header">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                    :data-bs-target="'#relatorioCollapse' + relatorio.id"
                                    aria-expanded="false"
                                    :aria-controls="'relatorioCollapse' + relatorio.id">
                                    {{ relatorio.dataConsulta }}
                                    
                                </button>
                            </h2>

                            <div :id="'relatorioCollapse' + relatorio.id"
                                class="accordion-collapse collapse" data-bs-parent="#accordionRelatorios">
                                <div class="accordion-body">
                                    <div class="d-flex mt-3 gap-2">
                            <label class="mt-2" for="ingredientesReceita">Medições Feitas</label>
                            <button type="button" @click="adicionarMedicao(relatorio)" class="btn btn-modal"><i
                        ...
                        <div v-for="(medicao, index) in relatorio.medicoes" class="mt-2" :key="index">
                                <div class="input-group">
                                    <input type="text" class="form-control" v-model="medicao.medicao.nome" placeholder="Medição" required>
                                    <input type="number" class="form-control" v-model="medicao.valor" step="0.1" placeholder="Valor medido" required>
                                    <input type="text" class="form-control" v-model="medicao.medicao.unidade" placeholder="Unidade" required>
                                    <div class="btn btn-danger" @click="removerMedicao(relatorio, medicao, index)"><i class="bi bi-trash-fill"></i></div>
                                </div>
                            </div>
                            <div class="d-flex flex-row align-items-end">
                                ...
                                <div class="input-group justify-content-end mt-3">
                                    <button class="btn btn-danger" @click="deletarRelatorio(relatorio, relatorioIndex)"><i class="bi bi-trash-fill"></i> Deletar</button>
                                    <button type="submit" class="btn btn-success"><i class="bi bi-check-lg"></i> Salvar Alterações</button>
                                </div>
                            </div>
                        </form>
                                </div>
